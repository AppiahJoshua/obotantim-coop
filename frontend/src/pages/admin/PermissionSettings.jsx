...
export default function PermissionSettings() {
  ...
  // 1. Fetch roles & safely parse objects or strings into clean role items
  const { data: roles = [] } = useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        // Normalize array items so we always have { id, label }
        return raw.map((r) => {
  ...
  // 2. Fetch permission rules matrix
  ...
  // 3. Single widget toggle mutation
  ...
  // Helper check if widget is visible for a role
